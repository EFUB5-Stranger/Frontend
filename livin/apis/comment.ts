...
// 댓글 목록 조회
export const getCommentsApi = async (review_id: number) => {
  const res = await axiosInstance.get(`/review/${review_id}/comment`);
  return res.data;
};

// 댓글 작성
export const createCommentApi = async (review_id: number, data: {
  content: string;
  anonymous: boolean;
}) => {
...
// 댓글 삭제
export const deleteCommentApi = async (comment_id: number) => {
